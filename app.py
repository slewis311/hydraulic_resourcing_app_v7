...
def require_login():
    ...
    with col2:
        st.caption("Set APP_PASSWORD in Streamlit secrets or environment")
...
def get_supabase():
    ...
    if not url or not key:
        st.error("Missing Supabase settings, set SUPABASE_URL and SUPABASE_ANON_KEY in secrets")
        st.stop()
    return create_client(url, key)

def db_init_tables(supabase):
...
def db_replace_member_settings(supabase, member, settings_row, leave_dates):
    # Upsert settings
    db_upsert_rows(supabase, "staff_settings", [settings_row], "member")
    # Replace leave dates for this member
    try:
        supabase.table("leave_days").delete().eq("member", member).execute()
    except Exception:
        pass
    if leave_dates is None:
        leave_dates = []
    leave_rows = [{"member": member, "leave_date": d} for d in leave_dates]
    if len(leave_rows) > 0:
        db_upsert_rows(supabase, "leave_days", leave_rows, "member,leave_date")

def load_from_db():
    supabase = get_supabase()

    team_rows = db_fetch_table(supabase, "team_members")
    jobs_rows = db_fetch_table(supabase, "jobs")
    settings_rows = db_fetch_table(supabase, "staff_settings")
    leave_rows = db_fetch_table(supabase, "leave_days")

    return supabase, team_rows, jobs_rows, settings_rows, leave_rows

def seed_if_empty(supabase):
    team_rows = db_fetch_table(supabase, "team_members")
    jobs_rows = db_fetch_table(supabase, "jobs")

    if len(team_rows) == 0:
        # Seed from existing session defaults if present
        default_team = st.session_state.get("team", None)
        if default_team is None or not hasattr(default_team, "to_dict"):
            default_team = []
        else:
            default_team = default_team.to_dict(orient="records")
        if len(default_team) == 0:
            default_team = [{"member":"SL","daily_hours":8.0},{"member":"LS","daily_hours":8.0},{"member":"LB","daily_hours":8.0}]
        db_upsert_rows(supabase, "team_members", default_team, "member")

    if len(jobs_rows) == 0:
        default_jobs = st.session_state.get("jobs_raw", None)
        if default_jobs is None or not hasattr(default_jobs, "to_dict"):
            default_jobs = []
        else:
            default_jobs = default_jobs.to_dict(orient="records")
        # Map to db schema
        mapped = []
...
def sync_session_from_db(team_rows, jobs_rows, settings_rows, leave_rows):
    ...
    if "Member" not in team_df.columns:
        if "member" in team_df.columns:
            team_df["Member"] = team_df["member"]
        else:
            team_df["Member"] = None
    if "Daily hours" not in team_df.columns:
        if "daily_hours" in team_df.columns:
            team_df["Daily hours"] = team_df["daily_hours"]
        else:
            team_df["Daily hours"] = 8.0
    team_df = team_df[["Member","Daily hours"]].copy()

    jobs_df = pd.DataFrame(jobs_rows) if len(jobs_rows) > 0 else pd.DataFrame(columns=["job_name","required_hours","priority","assignee","due_date","notes"])
    if len(jobs_df) > 0:
        jobs_df["Job name"] = jobs_df.get("job_name")
        jobs_df["Required hours"] = jobs_df.get("required_hours")
        jobs_df["Priority"] = jobs_df.get("priority")
        jobs_df["Assignee"] = jobs_df.get("assignee")
        jobs_df["Due date"] = pd.to_datetime(jobs_df.get("due_date"), errors="coerce").dt.date
        jobs_df["Notes"] = jobs_df.get("notes").fillna("")
        jobs_df = jobs_df[["Job name","Required hours","Priority","Assignee","Due date","Notes"]].copy()
    else:
    ...
    for r in leave_rows:
        ...
        if m is None or d is None:
            continue
        leave_map.setdefault(m, []).append(pd.to_datetime(d, errors="coerce").date())

    for r in settings_rows:
        m = r.get("member")
        if not m:
            continue
        weekdays = r.get("working_weekdays", [0,1,2,3,4])
        start_date = pd.to_datetime(r.get("start_date"), errors="coerce")
        start_date = start_date.date() if not pd.isna(start_date) else date.today()
        ms[m] = {
            "weekdays": set(int(x) for x in weekdays),
            "leave_dates": leave_map.get(m, []),
            "start_date": start_date,
        }

def persist_team_and_jobs(supabase):
    # Persist team
    team_df = st.session_state.get("team", None)
    ...
    # Persist jobs
    jobs_df = st.session_state.get("jobs_raw", None)
    if jobs_df is not None and hasattr(jobs_df, "to_dict"):
        rows = []
...
with tabs[1]:
    ...
    with left:
        ...
        settings_row = {"member": selected_member, "start_date": sdate, "working_weekdays": sorted(list(st.session_state["member_settings"][selected_member].get("weekdays", {0,1,2,3,4}))) }
        db_replace_member_settings(supabase, selected_member, settings_row, st.session_state["member_settings"][selected_member].get("leave_dates", []))
        ...
        settings_row = {"member": selected_member, "start_date": st.session_state["member_settings"][selected_member].get("start_date"), "working_weekdays": sorted(list(st.session_state["member_settings"][selected_member].get("weekdays", {0,1,2,3,4}))) }
        db_replace_member_settings(supabase, selected_member, settings_row, st.session_state["member_settings"][selected_member].get("leave_dates", []))
        ...
        settings_row = {
            ...
            "start_date": st.session_state["member_settings"][selected_member].get("start_date"),
            "working_weekdays": sorted(list(st.session_state["member_settings"][selected_member].get("weekdays", {0,1,2,3,4}))),
        }
        db_replace_member_settings(supabase, selected_member, settings_row, st.session_state["member_settings"][selected_member].get("leave_dates", []))
